...
func _process( _delta: float) -> void:
	if linear_velocity.sign().x ==1:
		fishVisual.flip_h = true
	elif linear_velocity.sign().x == -1:
		fishVisual.flip_h = false
...
func chaseFood(_delta: float) ->void:
	chasing = true
	destination.x = collision_shape_2d.get_global_mouse_position().x #add an offset like a circle 
	if chasing:
		linear_velocity.x =destination.x-position.x * speed
